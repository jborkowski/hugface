...
async fn main() -> Result<()> {
    ...
    let token = std::env::var("HF_TOKEN")
        .context("Failed to retrieve HuggingFace token. The environment variable 'HF_TOKEN' is not found.".to_string())?;

    let state = model::initialise_model(token)?;

    let router = Router::new().route("/prompt", post(run_pipeline)).with_state(state);
